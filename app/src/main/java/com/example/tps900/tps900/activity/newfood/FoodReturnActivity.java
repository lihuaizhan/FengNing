...
import com.example.tps900.tps900.activity.GoodsReturnBean;
import com.example.tps900.tps900.activity.MainActivity;
...
public class FoodReturnActivity extends AppCompatActivity {
    ...
    private int PrintdetailId;
    private int OldDetailId;
    ...
    private void findView() {
        ...
        et_returnGoods.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                et_returnGoods.setInputType(InputType.TYPE_NULL);
                KeyboardUtil keyboard = new KeyboardUtil(getApplicationContext(), FoodReturnActivity.this, et_returnGoods, R.id.schemas_key_keyboard_view);
                keyboard.setSusseccListener(new KeyboardUtil.OnListener() {
                    @Override
                    public void onSusseccLinister() {
    ...
    private void setListener() {
        ...
        img_back.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                FoodReturnActivity.this.startActivity(new Intent(FoodReturnActivity.this, MainActivity.class));
                FoodReturnActivity.this.finish();
            }
        });
        //扫描按妞点击事件
        btn_scan.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ClearData();
                Intent intent = new Intent(FoodReturnActivity.this, CaptureActivity.class);
                FoodReturnActivity.this.startActivityForResult(intent, 1002);
            }
        });
        ...
        btn_search.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (!TextUtils.isEmpty(orderNo)) {
                    //弹出进度条
                    DialogUtil.showDownloadDialog(FoodReturnActivity.this);
                    getReturnInfo(orderNo);
                } else {
                    ToastUtils.show(FoodReturnActivity.this, "请输入退货单号!");
                    return;
                }
            }
        });
            ...
            public void onClick(View view) {
                ...
                if (null != list_delete) {
                    if (!list_delete.isEmpty()) {
                        DialogUtil.showReturnDialog(FoodReturnActivity.this);
                        //退货集合中有客户要退货的商品
            ...
            try {
                ...
                if (isSuccess) {
                    ...
                    if (Double.parseDouble(shouldMOney) > 0.00) {
                        ...
                        FoodReturnActivity.this.runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                    ...
                    FoodReturnActivity.this.runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            DialogUtil.cancelDownloadDialog();
                            ShowInfo showInfo = new ShowInfo(FoodReturnActivity.this);
                            showInfo.infoDialog(FoodReturnActivity.this, "订单编号信息", "获取订单号失败");
                        }
                    });
                }
    ...
    private void submitReturnGoodsInfo(ArrayList<ReturnBean> list, double rDealId, int detailId, int payType, double money, double cardNo) {
        //原单号                              //退货单号
        String json = "{\"NRDEALID\"" + ":" + rDealId + ",\"NDEALID\"" + ":" + detailId + ",\"NGZONEID\"" + ":" + Constant.Food_ZONEID + ",\"NTERMINALID\"" + ":" + Constant.Food_TERMINALID + ",\"NEMPID\"" + ":" + Constant.EMPID + ",\"DDATE\"" + ":\"" + TimeUtils.time() + "\"" + ",\"NPAYMENT\"" + ":" + payType + ",\"NAMOUNT\"" + ":" + money + ",\"CommInfos\":";//订单明细信息
        PrintdetailId = detailId;
        OldDetailId = (int) rDealId;
        if (list.size() == 1) {
        ...
        String result = GsWebServiceUtils.submitFoodReturnInfo(json);
        LogUtil.i(TAG, "result退货----->" + result);
        if (!result.equals("err")) {
            GoodsReturnBean goodsReturnBean = JSON.parseObject(result, GoodsReturnBean.class);
            if (goodsReturnBean.isSuccess) {
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        try {
                            TpsN900PrintUtils.TpsN900ReturnPrint(FoodReturnActivity.this, list_delete, returnMoney, orderNo, String.valueOf(detailId), reTurnPayType);
                            FoodReturnActivity.this.runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    DialogUtil.cancelDownloadDialog();
                ...
                FoodReturnActivity.this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        ToastUtils.show(FoodReturnActivity.this, "提交退货信息失败!");
    ...
    private void parseJson(String result) {
        ...
        try {
            ...
            if (isSuccess) {
                //访问成功
                //订单ID
                ndealid = jsonObject.getInt("NDEALID");
                ...
                if (!flag) {
                    ShowInfo showInfo = new ShowInfo(FoodReturnActivity.this);
                    showInfo.infoDialog(FoodReturnActivity.this, "退货信息", "该订单已经退完货");
                    list.clear();
    ...
    public void getReturnInfo(final String scanResult) {
        ...
        Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                String result = GsWebServiceUtils.getFoodReturnInfo(String.valueOf(Constant.Food_ZONEID), scanResult);
                if (null != result) {
                    ...
                    FoodReturnActivity.this.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            ToastUtils.show(FoodReturnActivity.this, "获取退货订单信息失败!");
                            DialogUtil.cancelDownloadDialog();
                        }
                    });
                }
