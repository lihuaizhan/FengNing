...
public class DisOrderChangeTicketActivity extends Activity implements ShopcartAdapter.CheckInterface,
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        Constant.context = DisOrderChangeTicketActivity.this;
        initView();
    }
    ...
    public void initView() {
        ...
        progressDialog = new ProgressDialog(DisOrderChangeTicketActivity.this);
    ...
    public void onClick(View view) {
        int i = view.getId();
        if (i == R.id.ticket_et_ticketCode) {
            ticketEtTicketCode.setHint("请输入门票串码");
            new KeyboardUtil(getApplicationContext(), DisOrderChangeTicketActivity.this, ticketEtTicketCode, R.id.schemas_key_keyboard_view).showKeyboard();

        } else if (i == R.id.ticket_img_deleteCode) {
            ticketEtTicketCode.setText("");

        } else if (i == R.id.ticket_QueryTicket) {
        ...
        } else if (i == R.id.ticket_btn_bor) {
            Intent intent = new Intent(DisOrderChangeTicketActivity.this, CaptureActivity.class);
            startActivityForResult(intent, 100);

        } else if (i == R.id.ticket_btn_idcard) {
            DataAndClear(true, null);
            new GetIDInfoTask().execute();

        } else if (i == R.id.ticket_btn_comit) {
            ticket_list();
            if (ticketList.size() == 0) {
                ToastUtils.show(DisOrderChangeTicketActivity.this, "请选择要兑换的票");
            } else {
                ISPrintTicket(DisOrderChangeTicketActivity.this, "是否兑换此票");
            }

        } else if (i == R.id.ticket_lv_back) {
            DisOrderChangeTicketActivity.this.startActivity(new Intent(this, MainActivity.class));
            finish();

        } else if (i == R.id.all_chekbox) {
            doCheckAll();
    ...
    public void Webservice_GetDisOrder(final String Ecode, final String CardNo, final String OrderNo) {
        ...
        if (TextUtils.isEmpty(getDisOrder) || getDisOrder.equals("err")) {
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    ErrDialog2(DisOrderChangeTicketActivity.this, "请求数据失败 请检查网络或者检查配置", "");
                }
            });
            ...
            if (getDisODerBean.Success && getDisODerBean.getListData() != null) {
                ...
                if (getListDIsOrder.size() == 0) {
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            ErrDialog2(DisOrderChangeTicketActivity.this, "该订单已核销 ", "");
                        }
                    });
                } else {
                    runOnUiThread(new Runnable() {
                ...
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        ErrDialog2(DisOrderChangeTicketActivity.this, "查询票信息失败 ", "");
                    }
                });
            }
        }
        ...
        if (!TextUtils.isEmpty(disjson) && !disjson.equals("err")) {
            ...
            if (disOrderBean.Success) {
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            for (int i = 0; i < ticketList.size(); i++) {
                                TpsN900PrintUtils.TpsN900Print_ticket(DisOrderChangeTicketActivity.this,
                                        "散客兑票",
                ...
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        ErrDialog2(DisOrderChangeTicketActivity.this, "兑票失败" + "\n" + "该票已兑换并打印", "");
                        DataAndClear(true, null);
                    }
                });
            }
            ...
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    ErrDialog2(DisOrderChangeTicketActivity.this, "请求数据失败 请检查网络", "");
                    DataAndClear(true, null);
                }
            });
        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, final Intent data) {
        if (data != null) {
            ...
            Toast.makeText(DisOrderChangeTicketActivity.this, "扫描结果为空", Toast.LENGTH_LONG).show();
        }
    }
    ...
    public class IntentBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            switch (action) {
                case "Intent":
                    DisOrderChangeTicketActivity.this.startActivity(new Intent(DisOrderChangeTicketActivity.this, MainActivity.class));
                    finish();
                    break;
    ...
    private class GetIDInfoTask extends
        ...
        protected void onPreExecute() {
            ...
            dialog = new ProgressDialog(DisOrderChangeTicketActivity.this);
        ...
        protected TelpoException doInBackground(Void... arg0) {
            ...
            try {
                IdCard.open(DisOrderChangeTicketActivity.this);
                publishProgress(1);
        ...
        protected void onPostExecute(TelpoException result) {
            ...
            if (result == null && !info.getNo().equals("timeout")) {
                ...
                ToastUtils.show(DisOrderChangeTicketActivity.this, "读取身份证超时");
    ...
    private void initEvents() {
        selva = new ShopcartAdapter(groups, children, DisOrderChangeTicketActivity.this);
    ...
    protected void onDestroy() {
        super.onDestroy();
        IdCard.close();
        DisOrderChangeTicketActivity.this.unregisterReceiver(ibr);
    }
